...
def load_polynomial_dataset():
    print("Loading Polynomial Function Approximation dataset...")
...
def evaluate_joint_config(optimizer_class, algorithm_name: str, params: Dict,
                        ...
                        convergence_threshold: float = 0.05, use_early_stopping: bool = True):
...
def phase1_coarse_search(X: np.ndarray, y: np.ndarray):
    ...
    with open(results_dir / "polynomial_phase1_all.pkl", "wb") as f:
...
def phase2_fine_search_per_algorithm(phase1_results: List[JointResult], X: np.ndarray, y: np.ndarray):
    ...
    for alg_name, best_phase1 in best_per_algorithm.items():
        ...
        for h in fine_architectures:
            ...
            result_dict = evaluate_joint_config(
                alg_class, alg_name, best_params, h, X, y, max_epochs=2000, use_early_stopping=early_stopping_map[alg_name]
            )
...
def main():
    ...
    print("Polynomial Function Approximation (PyTorch Version)")
    print("="*60)

    X, y = load_polynomial_dataset()
    ...
    best_overall = sorted_algorithms[0][1]

    with open(results_dir / "joint_polynomial_optimization.txt", "w") as f:
        f.write("JOINT HYPERPARAMETER AND ARCHITECTURE OPTIMIZATION - POLYNOMIAL (PyTorch)\n")
        f.write("="*70 + "\n\n")
    ...
    with open(results_dir / "polynomial_convergence_data.pkl", "wb") as f:
        pickle.dump(convergence_data, f)

    print(f"\nResults saved to: {results_dir / 'joint_polynomial_optimization.txt'}")
    print(f"Convergence data saved to: {results_dir / 'polynomial_convergence_data.pkl'}")
