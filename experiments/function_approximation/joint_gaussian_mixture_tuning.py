...
def load_gaussian_mixture_dataset():
    print("Loading Gaussian Mixture Function Approximation dataset...")
...
def evaluate_joint_config(optimizer_class, algorithm_name: str, params: Dict,
                        ...
                        n_folds: int = 10, max_epochs: int = 2000, seed: int = 42,
                        convergence_threshold: float = 0.15, use_early_stopping: bool = True):
    ...
    for fold_idx, (train_idx, val_idx) in enumerate(kfold.split(X, y)):
        ...
        try:
            ...
            val_pred_unscaled = val_pred * y_std + y_mean
            rmse = np.sqrt(np.mean((val_pred_unscaled - y_val)**2))
...
def phase1_coarse_search(X: np.ndarray, y: np.ndarray):
        ...
        for params in param_combinations:
            for h in architectures:
                ...
                result_dict = evaluate_joint_config(
                    alg_class, alg_name, params, h, X, y, max_epochs=2000,
                    use_early_stopping=use_early_stopping
                )

                result = JointResult(
                    algorithm=alg_name, params=params, hidden_dim=h,
                    rmse_mean=result_dict['rmse_mean'],
                    rmse_std=result_dict['rmse_std'],
                ...
                if result.rmse_mean != float('inf'):
                    conv_info = ""
                    if result.convergence_epoch_mean is not None:
                        conv_info = f" | Conv: {result.convergence_epoch_mean:.1f} epochs, {result.convergence_time_mean:.2f}s"
                    print(f"    RMSE: {result.rmse_mean:.4f} ± {result.rmse_std:.4f}{conv_info}")
                else:
                    print(f"    FAILED")
    ...
    with open(results_dir / "gaussian_phase1_all.pkl", "wb") as f:
    ...
    valid_results = [r for r in results if r.rmse_mean != float('inf')]
    if not valid_results:
        raise ValueError("All configurations failed!")

    best = min(valid_results, key=lambda r: r.rmse_mean)
    print(f"\nBest Phase 1 Result:")
...
def phase2_fine_search_per_algorithm(phase1_results: List[JointResult], X: np.ndarray, y: np.ndarray):
    ...
    for alg in algorithms:
        alg_results = [r for r in phase1_results if r.algorithm == alg and r.rmse_mean != float('inf')]
        if alg_results:
            best_per_algorithm[alg] = min(alg_results, key=lambda r: (r.rmse_mean, r.rmse_std, r.hidden_dim))

    print(f"Best Phase 1 results per algorithm:")
    for alg, best in best_per_algorithm.items():
        print(f"  {alg.upper()}: h={best.hidden_dim}, RMSE={best.rmse_mean:.4f}±{best.rmse_std:.4f}")
    ...
    for alg_name, best_phase1 in best_per_algorithm.items():
        ...
        for h in fine_architectures:
            ...
            result_dict = evaluate_joint_config(
                alg_class, alg_name, best_params, h, X, y, max_epochs=2000,
                use_early_stopping=early_stopping_map[alg_name]
            )
        ...
        valid_results = [r for r in phase2_results if r.rmse_mean != float('inf')]
        if valid_results:
            best_alg_final = min(valid_results, key=lambda r: (r.rmse_mean, r.rmse_std, r.hidden_dim))
            final_results[alg_name] = best_alg_final
            print(f"  Best {alg_name.upper()}: h={best_alg_final.hidden_dim}, RMSE={best_alg_final.rmse_mean:.4f}±{best_alg_final.rmse_std:.4f}")
        else:
...
def main():
    ...
    print("Gaussian Mixture Function Approximation (PyTorch Version)")
    print("="*60)

    X, y = load_gaussian_mixture_dataset()
    ...
    sorted_algorithms = sorted(algorithm_results.items(), key=lambda x: x[1].rmse_mean)

    print(f"{'Rank':<4} {'Algorithm':<12} {'Hidden':<7} {'RMSE':<15} {'Conv_Epoch':<12} {'Conv_Time(s)':<14} {'Total_Time(s)':<14}")
    print("-" * 100)
    ...
    for rank, (alg_name, result) in enumerate(sorted_algorithms, 1):
        ...
        print(f"{rank:<4} {alg_name.upper():<12} {result.hidden_dim:<7} "
              f"{result.rmse_mean:.4f}±{result.rmse_std:.4f} {conv_epoch_str:<12} {conv_time_str:<14} {total_time_str:<14}")
    ...
    print(f"  Final RMSE: {best_result.rmse_mean:.4f} ± {best_result.rmse_std:.4f}")

    results_dir = Path("results/hyperparameter_tuning")
    results_dir.mkdir(parents=True, exist_ok=True)

    with open(results_dir / "joint_gaussian_mixture_optimization.txt", "w") as f:
        f.write("JOINT HYPERPARAMETER AND ARCHITECTURE OPTIMIZATION - GAUSSIAN MIXTURE (PyTorch)\n")
        f.write("="*75 + "\n\n")
        ...
        f.write(f"  RMSE: {best_result.rmse_mean:.4f} ± {best_result.rmse_std:.4f}\n\n")

        f.write("Per-Algorithm Final Results:\n")
        for rank, (alg_name, result) in enumerate(sorted_algorithms, 1):
            f.write(f"  {rank}. {alg_name.upper()}: h={result.hidden_dim}, ")
            f.write(f"RMSE={result.rmse_mean:.4f}±{result.rmse_std:.4f}\n")
        ...
        valid_phase1 = [r for r in phase1_results if r.rmse_mean != float('inf')]
        sorted_phase1 = sorted(valid_phase1, key=lambda r: r.rmse_mean)

        for i, result in enumerate(sorted_phase1[:10]):  # Top 10
    ...
    with open(results_dir / "gaussian_mixture_convergence_data.pkl", "wb") as f:
        pickle.dump(convergence_data, f)

    print(f"\nResults saved to: {results_dir / 'joint_gaussian_mixture_optimization.txt'}")
    print(f"Convergence data saved to: {results_dir / 'gaussian_mixture_convergence_data.pkl'}")
