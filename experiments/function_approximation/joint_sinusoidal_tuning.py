...
def load_sinusoidal_dataset():
    print("Loading Sinusoidal Function Approximation dataset...")
...
def evaluate_joint_config(optimizer_class, algorithm_name: str, params: Dict,
                        ...
                        n_folds: int = 10, max_epochs: int = 5000, seed: int = 42,
                        convergence_threshold: float = 0.3, use_early_stopping: bool = True):
...
def phase1_coarse_search(X: np.ndarray, y: np.ndarray):
    ...
    with open(results_dir / "sinusoidal_phase1_all.pkl", "wb") as f:
...
def main():
    ...
    print("Sinusoidal Function Approximation (PyTorch Version)")
    print("="*60)

    X, y = load_sinusoidal_dataset()
    ...
    with open(results_dir / "joint_sinusoidal_optimization.txt", "w") as f:
        f.write("JOINT HYPERPARAMETER AND ARCHITECTURE OPTIMIZATION - SINUSOIDAL (PyTorch)\n")
        f.write("="*70 + "\n\n")
    ...
    with open(results_dir / "sinusoidal_convergence_data.pkl", "wb") as f:
        pickle.dump(convergence_data, f)

    print(f"\nResults saved to: {results_dir / 'joint_sinusoidal_optimization.txt'}")
    print(f"Convergence data saved to: {results_dir / 'sinusoidal_convergence_data.pkl'}")
