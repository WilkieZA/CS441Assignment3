...
def load_iris_dataset():
    print("Loading Iris Classification dataset...")
...
def phase1_coarse_search(X: np.ndarray, y: np.ndarray):
    ...
    with open(results_dir / "iris_phase1_all.pkl", "wb") as f:
...
def main():
    ...
    print("Iris Species Classification (PyTorch)")
    print("="*60)
    ...
    with open(results_dir / "joint_iris_optimization.txt", "w") as f:
        f.write("JOINT HYPERPARAMETER AND ARCHITECTURE OPTIMIZATION - IRIS\n")
        f.write("="*60 + "\n\n")
    ...
    with open(results_dir / "iris_convergence_data.pkl", "wb") as f:
        pickle.dump(convergence_data, f)

    print(f"\nResults saved to: {results_dir / 'joint_iris_optimization.txt'}")
    print(f"Convergence data saved to: {results_dir / 'iris_convergence_data.pkl'}")
