...
def load_beer_dataset():
    print("Loading Beer Style Classification dataset...")
...
def phase1_coarse_search(X: np.ndarray, y: np.ndarray):
    ...
    algorithms = {
        'sgd': (SGDOptimizer, {'lr': [0.001, 0.01, 0.05, 0.1], 'momentum': [0.0, 0.5, 0.9, 0.99]}, True),
        'scg': (SCGOptimizer, {'sigma': [1e-6, 1e-5], 'lambd': [1e-3, 1e-2]}, False),
        'leapfrog': (LeapFrogOptimizer, {'dt': [1e-4, 1e-3], 'delta_max': [0.1, 1.0],
                                         'xi': [0.1, 0.2], 'm': [3, 5]}, False)
    }
    ...
    with open(results_dir / "beer_phase1_all.pkl", "wb") as f:
...
def phase2_fine_search_per_algorithm(phase1_results: List[JointResult], X: np.ndarray, y: np.ndarray):
    ...
    for alg in algorithms:
        ...
        if alg_results:
            # Sort by: 1) accuracy (desc), 2) std (asc), 3) hidden_dim (asc)
...
def main():
    ...
    print("Beer Style Classification (PyTorch)")
    print("="*60)
    ...
    # Phase 2: Fine search per algorithm
    ...
    with open(results_dir / "joint_beer_optimization.txt", "w") as f:
        f.write("JOINT HYPERPARAMETER AND ARCHITECTURE OPTIMIZATION - BEER\n")
        f.write("="*60 + "\n\n")
    ...
    with open(results_dir / "beer_convergence_data.pkl", "wb") as f:
        pickle.dump(convergence_data, f)

    print(f"\nResults saved to: {results_dir / 'joint_beer_optimization.txt'}")
    print(f"Convergence data saved to: {results_dir / 'beer_convergence_data.pkl'}")
